...
namespace Lgy.Km.Tests
{
    public abstract class KmTestBase : AbpIntegratedTestBase<KmTestModule>
    {
        ...
        protected void UsingDbContext(Action<KmDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<KmDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<KmDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<KmDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<KmDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<KmDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<KmDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<KmDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<KmDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<KmDbContext, Task<T>> func)
        {
            T result;
